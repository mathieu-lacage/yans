...
TclTSpecRequest::TclTSpecRequest (TSpec *tspec, char const *tclTspec,
	...
	: TSpecRequest (tspec),
	  m_tclTspec (tclTspec),
	  m_grantedCallback (grantedCallback),
	  m_refusedCallback (refusedCallback)
{}
...
TclTSpecRequest::notifyRequestGranted (void)
{
	...
	tcl.evalf ("%s %s %d", m_grantedCallback.c_str (), m_tclTspec.c_str (), getTSpec ()->getTSID ()+1);
}
void 
TclTSpecRequest::notifyRequestRefused (void)
{
	Tcl& tcl=Tcl::instance();
	tcl.evalf ("%s %s %d", m_refusedCallback.c_str (), m_tclTspec.c_str (), getTSpec ()->getTSID ()+1);
}
